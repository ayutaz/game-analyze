# 日本市場の有名ゲーム100本 — 3カテゴリ分類カタログ

本リポジトリは、国内（日本）で発売・配信され、一定規模の販売・DL本数を記録した、または SNS でバズった有名ゲーム約100本を、以下3つの分類軸で整理した分析カタログです。
...
- **総タイトル数**: 99本（当初100本を目標としたが ID #56 が重複により欠番、正味99件 — `data/games.json` 基準）
- **主分類の内訳**（`data/games.json` 集計）:
  - 体験型 (EXP): **55本**
  - 物語型 (NAR): **23本**
  - 報酬型 (REW): **21本**
- **体験型の「他者軸」内訳**: 対戦 26 / 協力 10 / ソロ 5 / 非同期 4 / 非対称 1 / 未設定 9
- **プラットフォーム分布**: Switch / Switch 2 / PS4 / PS5 / 3DS / Wii / PC / Mobile / Arcade / Multi
